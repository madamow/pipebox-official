...
def run(args): 
    ...
    for enb in sorted(allexpnumnband):
        ...
        if yesorno != 0:
            ...
            if args.queue_size:
                not_queued = pipeutils.less_than_queue(pipeline='firstcut',user=args.jira_user,queue_size = args.queue_size)
            else:
                not_queued = pipeutils.less_than_queue(pipeline='firstcut',user=args.jira_user)
            if not_queued:
                count_unsubmitted += 1
